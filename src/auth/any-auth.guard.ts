...
export class AnyAuthGuard implements CanActivate {
    ...
    private async validateBasic(encoded: string, request: any): Promise<boolean> {
        ...
        const admin = await this.prisma.userAdmin.findUnique({ where: { email: credentials.email } });
    ...
    private decodeCredentials(encoded: string): { email: string; password: string } | null {
        try {
            ...
            return { email: decoded.slice(0, separatorIndex), password: decoded.slice(separatorIndex + 1) };
        } catch {
            return null;
        }
    }
