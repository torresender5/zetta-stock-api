...
import { CreatePurchaseDto, UpdatePurchasePaymentStatusDto } from './dto/purchase.dto';

@Controller('purchases')
export class PurchaseController {
    constructor(private purchaseService: PurchaseService, @Inject(WINSTON_MODULE_PROVIDER) private readonly logger: Logger) {}
    ...
    @HttpCode(HttpStatus.OK)
    @Patch(':id')
    updatePaymentStatus(@Param('id') id: string, @Body() data: UpdatePurchasePaymentStatusDto) {
        const purchaseId = parseInt(id, 10);
        this.logger.info(`Starting PurchaseController Update Payment Status: ${purchaseId}`)
        return this.purchaseService.updatePaymentStatus(purchaseId, data.paymentStatus);
    }
}
